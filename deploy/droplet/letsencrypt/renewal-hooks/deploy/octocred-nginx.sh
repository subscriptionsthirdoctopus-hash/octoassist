#!/bin/bash
# /etc/letsencrypt/renewal-hooks/deploy/octocred-nginx.sh
#
# Runs after every successful Let's Encrypt renewal that includes
# octocred.thirdoctopus.com. Re-copies the new cert into the
# hrms-nginx-readable path and reloads nginx (zero downtime).
set -e
DOMAIN="octocred.thirdoctopus.com"
NGX_SSL_DIR="/opt/hrms-erp/nginx/ssl/octocred"
...
case "$RENEWED_DOMAINS" in
  *"$DOMAIN"*)
    ...
    logger -t octocred-cert "renewed + nginx reloaded for $DOMAIN"
    ;;
esac
